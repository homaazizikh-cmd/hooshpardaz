  ...
  >
    <div class="container mx-auto px-4 flex justify-between items-center">
      ...
      <div class="flex justify-end items-center gap-3 z-50 w-auto md:w-1/4">
        ...
        <template v-else>
          <NuxtLink v-if="isAdmin" to="/admin" class="hidden md:flex items-center justify-center px-5 py-2.5 text-sm font-bold text-white bg-purple-600 hover:bg-purple-700 rounded-xl transition-all shadow-lg shadow-purple-600/30 whitespace-nowrap transform hover:-translate-y-0.5">
            ⚙️ پنل مدیریت
          </NuxtLink>

          <NuxtLink to="/dashboard" class="hidden md:flex items-center justify-center px-5 py-2.5 text-sm font-bold text-blue-700 bg-blue-100 hover:bg-blue-200 dark:text-blue-400 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 rounded-xl transition-all whitespace-nowrap">
            پنل کاربری
          </NuxtLink>
        </template>
  ...
  <aside :class="['fixed top-0 right-0 h-full w-64 bg-white dark:bg-brand-dark shadow-2xl z-40 transform transition-transform duration-300 ease-in-out md:hidden flex flex-col pt-24 border-l border-gray-200 dark:border-gray-800', isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full']" dir="rtl">
    ...
    <div class="px-4 pb-4 border-b border-gray-100 dark:border-gray-800 mb-2">
      ...
      <div v-else class="flex flex-col gap-2">
        <NuxtLink v-if="isAdmin" to="/admin" @click="toggleMobileMenu" class="flex items-center justify-center w-full py-3 text-sm font-bold text-white bg-purple-600 hover:bg-purple-700 rounded-xl transition shadow-md shadow-purple-600/20">
          ⚙️ پنل مدیریت
        </NuxtLink>
...
// ⚠️ ایمیل خودتان را دقیقاً در خط زیر به جای ایمیل تستی قرار دهید
const ADMIN_EMAIL = 'homaazizikh@gmail.com' 

// بررسی اینکه آیا کاربر فعلی ادمین است یا خیر
