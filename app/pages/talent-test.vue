<template>
  <div class="min-h-screen bg-gray-50 dark:bg-brand-dark transition-colors duration-500 py-12 px-4 relative overflow-hidden flex items-center justify-center">
    ...
    <div class="w-full max-w-4xl relative z-10">
      ...
      <div class="relative min-h-[400px] flex items-center justify-center">
        <Transition name="slide-fade" mode="out-in">
          ...
          <div v-if="!showResult && !isAnalyzing" :key="currentStep" 
               class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl p-8 md:p-12 rounded-[2.5rem] shadow-[0_20px_50px_-15px_rgba(0,0,0,0.1)] border border-white/40 dark:border-gray-700 w-full max-w-3xl mx-auto">
          ...
          <div v-else-if="isAnalyzing" key="analyzing" class="w-full flex flex-col items-center justify-center text-center py-20">
            ...
            <h2 class="text-2xl font-bold text-gray-800 dark:text-white mb-2 animate-pulse">در حال تحلیل الگوهای شناختی شما...</h2>
          ...
          <div v-else key="result" class="bg-white/90 dark:bg-gray-800/90 backdrop-blur-2xl p-8 md:p-12 rounded-[2.5rem] shadow-2xl border border-gray-100 dark:border-gray-700 w-full animate-fade-in">
            ...
            <div class="grid lg:grid-cols-2 gap-12 items-center mb-10">
              ...
              <div class="text-right">
                ...
                <p class="text-gray-600 dark:text-gray-300 mb-8 leading-loose text-justify font-medium">
                  {{ finalResult.desc }}
                </p>
                
                <div class="flex flex-col sm:flex-row items-center gap-4">
                  <NuxtLink :to="finalResult.link" class="w-full sm:w-auto inline-flex justify-center bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-2xl font-bold transition-all transform hover:-translate-y-1 shadow-[0_10px_20px_-10px_rgba(37,99,235,0.6)]">
                    شروع مسیر اختصاصی شما
                  </NuxtLink>
...
// محاسبات مختصات نقاط رادار (با اضافه کردن یک مینیمم مقدار تا نمودار به هم نریزد)
const logicPoint = computed(() => {
  const r = Math.max(scores.value.logic * 10, 5); // شعاع از ۵ تا ۱۰۰
...
const answer = (points) => {
  const currentCat = questions[currentStep.value].cat;
  scores.value[currentCat] += points;
  
  if (currentStep.value < questions.length - 1) {
    // ایجاد تاخیر برای دیده شدن افکت کلیک
    setTimeout(() => {
      currentStep.value++;
    }, 150);
  } else {
    // پایان تست -> نمایش صفحه پردازش
    setTimeout(() => {
...
const calculateFinalResult = () => {
  ...
  // شبیه‌سازی زمان تحلیل برای اثر روانی بهتر
  setTimeout(() => {
    if (maxScore === scores.value.logic) {
      finalResult.value = {
        title: "تحلیل‌گر و معمار سیستم (برنامه‌نویس)",
        desc: "پاسخ‌های شما نشان می‌دهد ذهن شما به شدت ساختاریافته، تحلیلی و منطقی است. شما از کشف الگوها در میان اطلاعات پیچیده لذت می‌برید. دنیای برنامه‌نویسی، هوش مصنوعی و حل مسائل الگوریتمی دقیقاً جایی است که می‌توانید در آن به یک متخصص غیرقابل جایگزین تبدیل شوید.",
        link: "/courses"
      };
    } else if (maxScore === scores.value.creative) {
      finalResult.value = {
        title: "خالق بصری و دیزاینر (UI/UX)",
        desc: "شما دنیا را از دریچه زیبایی، فرم و کاربردپذیری می‌بینید. نیمکره راست ذهن شما به شدت فعال است. شما نه تنها می‌توانید محصولات دیجیتال بسازید، بلکه می‌دانید چگونه آن‌ها را برای انسان‌ها جذاب و دوست‌داشتنی کنید. طراحی رابط و تجربه کاربری مسیر اصلی شماست.",
        link: "/courses"
      };
    } else {
      finalResult.value = {
        title: "مهندس عمل‌گرا و خالق سیستم‌ها",
        desc: "شما یک مهندس ذاتی هستید! کنجکاوی شما برای درک «نحوه کارکرد اشیاء» و علاقه به خلق چیزهای ملموس، نقطه قوت شماست. ترکیب کدهای برنامه‌نویسی با سخت‌افزار، اینترنت اشیا (IoT) و رباتیک مسیری است که شما را به شدت به وجد خواهد آورد.",
        link: "/courses"
      };
    }
    
    // اتمام پردازش و نمایش نتیجه
    isAnalyzing.value = false;
    showResult.value = true;
  }, 2500); // 2.5 ثانیه مکث برای تحلیل
};
